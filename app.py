...
st.write("005930(삼성전자) 처럼 주식 종목 코드 6자리를 입력하세요.")
...
# ⚙️ 설정값 (사이드바에 숨김 - 주식용 최적값 적용)
with st.sidebar:
    st.header("⚙️ 전략 설정")
    ATR_PERIOD = st.slider("ATR 기간", 10, 30, 20)
    ATR_FACTOR = st.slider("진입 Factor", 1.0, 5.0, 2.0, 0.1)      # 주식용 추천 2.0
    ATR_STOP_MULT = st.slider("청산 Multiplier", 1.0, 5.0, 2.5, 0.1) # 주식용 추천 2.5
